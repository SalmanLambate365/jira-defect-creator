...
if st.button("🚀 Create Defect"):
    ...
    evidence_names = [f.name for f in (uploaded_files or [])]
    summary = copied["text"]["issue_description"] or f"Issue in {test_ticket.strip()}"
    ...
    edit_fields = {}
    if copied["labels"]:      edit_fields["labels"] = [lbl for lbl in copied["labels"] if lbl != "JiraTestGenAI"]
...
auth = get_auth()
# Update Test ticket status to Fail
transition_payload = {"transition": {"id": "31"}}  # Replace 31 with actual Fail transition ID
transition_resp = requests.post(
    f"{JIRA_BASE_URL}/rest/api/3/issue/{test_ticket.strip()}/transitions",
...
if transition_resp.status_code not in (200, 204):
    st.warning(f"Failed to update Test ticket status: {transition_resp.status_code} {transition_resp.text[:300]}")



auth = get_auth()
# Link defect to Zephyr Test Execution and fail execution
try:
    execution_id = "<fetch_execution_id_logic_here>"  # Replace with actual logic to fetch execution ID
    zephyr_link_payload = {"issueId": jira_issue_id_from_key(issue_key)}
    zephyr_link_resp = requests.post(
        f"{ZEPHYR_BASE}/public/rest/api/1.0/execution/{execution_id}/defects",
        headers={"Authorization": f"JWT {build_zephyr_jwt('POST', f'/public/rest/api/1.0/execution/{execution_id}/defects')}", "zapiAccessKey": st.secrets['ZEPHYR_ACCESS_KEY'], "Content-Type": "application/json"},
        json=zephyr_link_payload,
        timeout=30
    )
    if zephyr_link_resp.status_code not in (200, 201):
        st.warning(f"Failed to link defect to Zephyr execution: {zephyr_link_resp.status_code} {zephyr_link_resp.text[:300]}")

    # Fail the execution status
    fail_payload = {"status": {"id": 2}}  # 2 = Fail in Zephyr
    fail_resp = requests.put(
        f"{ZEPHYR_BASE}/public/rest/api/1.0/execution/{execution_id}",
        headers={"Authorization": f"JWT {build_zephyr_jwt('PUT', f'/public/rest/api/1.0/execution/{execution_id}')}", "zapiAccessKey": st.secrets['ZEPHYR_ACCESS_KEY'], "Content-Type": "application/json"},
        json=fail_payload,
        timeout=30
    )
    if fail_resp.status_code not in (200, 204):
        st.warning(f"Failed to update Zephyr execution status: {fail_resp.status_code} {fail_resp.text[:300]}")
except Exception as e:
    st.warning(f"Zephyr linking failed: {e}")
